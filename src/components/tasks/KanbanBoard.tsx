...
const DropZone: React.FC<DropZoneProps> = ({ status, index, onDrop, isEmptyColumn, onAddTask }) => {
  ...
  if (isEmptyColumn) {
    return (
      <div ref={drop} className="h-full min-h-[160px]">
        <div
          className={`flex flex-col items-center justify-center h-full border-2 border-dashed rounded-xl cursor-pointer group ${
            isActive
  ...
  // FIXED: Larger hit target (h-4 = 16px minimum) and instant visual feedback (50ms transitions)
  return (
    <div
      ref={drop}
      className="relative"
      style={{
        ...
        transition: 'height 50ms ease-out, margin 50ms ease-out',
      }}
    >
      {/* Always visible subtle hit area */}
      <div
        className={`absolute inset-0 rounded-lg ${
          isActive
            ? 'bg-gradient-to-r from-brand-500/15 via-brand-500/25 to-brand-500/15 dark:from-brand-400/15 dark:via-brand-400/25 dark:to-brand-400/15 border-2 border-dashed border-brand-500 dark:border-brand-400'
            : 'bg-transparent'
        }`}
        style={{ transition: 'background 50ms, border-color 50ms' }}
      >
        {isActive && (
...
const DraggableTaskCard: React.FC<DraggableTaskCardProps> = ({
  ...
  return (
    <div
      ref={ref}
      className={`${isSubtask ? 'ml-8' : ''}`}
      style={{
        opacity: isDragging ? 0.4 : 1,
        transform: isDragging ? 'scale(0.98) rotate(5deg)' : 'scale(1) rotate(0deg)',
...
export const ColumnMenu: React.FC<ColumnMenuProps> = ({
  ...
  return (
    <div
      ref={menuRef}
      className="absolute right-0 top-full mt-2 w-48 bg-white dark:bg-gray-800 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700 py-1 z-50 animate-in fade-in slide-in-from-top-2 "
    >
      <button
        onClick={() => {
          onSortByPriority();
...
export const KanbanColumn: React.FC<KanbanColumnProps> = ({
  ...
  return (
    <div
      className="flex flex-col min-w-[280px] max-w-[280px] sm:min-w-[320px] sm:max-w-[320px] bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 shadow-sm hover:shadow-md"
      ...
      <div className="flex items-center justify-between p-3 border-b border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50 rounded-t-xl">
        ...
        <div className="flex items-center gap-0.5 flex-shrink-0">
          <button
            ...
            className="p-1.5 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-100 transition-all duration-120 hover:scale-105 active:scale-95"
            title="Add task"
          >
  ...
  return (
    <div className="flex flex-col h-full">
      {hiddenColumns.size > 0 && (
        <div className="flex items-center gap-3 mb-4 px-4 py-2.5 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900/50 rounded-lg animate-in slide-in-from-top duration-150">
