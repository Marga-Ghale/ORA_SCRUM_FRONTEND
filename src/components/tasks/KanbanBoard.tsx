...
const DropZone: React.FC<DropZoneProps> = ({ status, index, onDrop, isEmptyColumn, onAddTask }) => {
  ...
  if (isEmptyColumn) {
    return (
      <div ref={drop} className="h-full min-h-[120px]">
        <div
          className={`flex flex-col items-center justify-center h-full border-2 border-dashed rounded-lg cursor-pointer group ${
            isActive
              ? 'border-brand-400 dark:border-brand-500 bg-brand-50/50 dark:bg-brand-950/30'
          ...
          style={{ transition: 'border-color 50ms, background-color 50ms' }}
          onClick={onAddTask}
        >
          <div
            ...
            style={{ transition: 'background-color 50ms' }}
          >
            <Plus
  ...
  // Between tasks drop zone
  return (
    <div
      ref={drop}
      className="relative"
      style={{
        height: isActive ? '80px' : '15px',
        marginTop: isActive ? '12px' : '2px',
        marginBottom: isActive ? '12px' : '2px',
        transition: 'height 0ms, margin 0ms', // Changed from 50ms to 0ms for instant response
        transform: isActive ? 'rotate(-3deg) scale(1.02)' : 'rotate(0deg) scale(1)',
      }}
    >
      <div
        className={`absolute inset-0 rounded-md ${
          isActive
            ? 'bg-green-50/80 dark:bg-green-950/30 border-2 border-green-400/60 dark:border-green-500/50'
            : 'bg-transparent'
        }`}
      ...
      >
        {isActive && (
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="w-full h-0.5 bg-gradient-to-r from-transparent via-green-400/70 dark:via-green-500/50 to-transparent" />
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <div
      ...
      style={{
        ...
        transform: isDragging ? 'rotate(5deg) scale(1.02)' : 'rotate(0deg) scale(1)',
      }}
    >
      <div
        style={{
...
export const ColumnMenu: React.FC<ColumnMenuProps> = ({
  ...
  return (
    <div
      ref={menuRef}
      className="absolute right-0 top-full mt-1.5 w-44 bg-white dark:bg-gray-800 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700 py-1 z-50 animate-in fade-in slide-in-from-top-2"
    >
      <button
        onClick={() => {
          onSortByPriority();
        ...
        className="w-full px-3 py-1.5 text-left text-xs text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700/50 flex items-center gap-2 transition-colors"
      >
...
export const KanbanColumn: React.FC<KanbanColumnProps> = ({
  ...
  return (
    <div
      className="flex flex-col min-w-[260px] max-w-[260px] sm:min-w-[290px] sm:max-w-[290px] bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800 shadow-sm hover:shadow-md"
      style={{ transition: 'box-shadow 150ms' }}
    >
      {/* Column Header */}
      <div className="flex items-center justify-between px-2.5 py-2 border-b border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50 rounded-t-lg">
        <div className="flex items-center gap-1.5 min-w-0 flex-1">
          <div
            className="w-2 h-2 rounded-full flex-shrink-0 shadow-sm"
            style={{ backgroundColor: color }}
          />
          <h3 className="font-semibold text-gray-900 dark:text-white text-xs truncate">{name}</h3>
          <span className="flex items-center justify-center min-w-[18px] h-4 px-1.5 rounded-full bg-gray-200 dark:bg-gray-700 text-[10px] font-bold text-gray-700 dark:text-gray-300">
            {tasks.length}
          </span>
          {isUpdating && (
            <Loader2 className="w-3 h-3 animate-spin text-brand-500 dark:text-brand-400 flex-shrink-0" />
          )}
        </div>
        <div className="flex items-center gap-0.5 flex-shrink-0">
          <button
            onClick={() => onAddTask(status)}
            className="p-1 rounded-md hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-100 transition-all duration-120 hover:scale-105 active:scale-95"
            title="Add task"
          >
      ...
      <div
        ref={columnRef}
        className={`flex-1 p-2.5 overflow-y-auto kanban-scrollbar ${
  ...
  return (
    <div className="flex flex-col h-full">
      {hiddenColumns.size > 0 && (
        <div className="flex items-center gap-2.5 mb-3 px-3 py-2 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900/50 rounded-lg animate-in slide-in-from-top duration-150">
          ...
          <button
            ...
            className="ml-auto text-xs font-semibold text-amber-600 dark:text-amber-400 hover:text-amber-700 dark:hover:text-amber-300 transition-colors"
          >
            Show All
          </button>
        </div>
      ...
      <div className="flex gap-3 overflow-x-auto pb-3 kanban-scrollbar">
