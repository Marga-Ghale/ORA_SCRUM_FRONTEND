...
const MembersList: React.FC<MembersListProps> = ({ type, targetId, onInvite }) => {
  ...
  const { data: members, isLoading, error } = type === 'workspace' ? workspaceMembers : projectMembers;
            ...
            >
              ...
              <div className="flex items-center gap-2">
                {isAdmin && member.role !== 'owner' && member.user.id !== currentUser?.id ? (
                  ...
                  <span className={`px-2.5 py-1 text-xs font-medium rounded-lg ${ROLE_COLORS[member.role]}`}>
