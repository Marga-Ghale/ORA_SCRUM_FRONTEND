    ...
    return (
      ...
      >
        ...
        {isDM ? (
          ...
          >
            <Hash
              className={`w-3.5 h-3.5 ${isActive ? 'text-brand-400' : 'text-gray-500 dark:text-[#6b7280]'}`}
            />
          </div>
        )}
    ...
    return (
      <div className="flex items-center gap-1 px-2 py-1.5 group">
        <button
          ...
          className="flex items-center gap-1.5 text-xs font-semibold text-gray-500 dark:text-[#6b7280] uppercase tracking-wider hover:text-gray-700 dark:hover:text-[#9ca3af] transition-colors"
        >
          <ChevronRight
        ...
        <span className="text-[10px] text-gray-500 dark:text-[#6b7280] px-1.5 py-0.5 bg-gray-100 dark:bg-[#25282c] rounded ml-1">
          {count}
        </span>
    ...
    <div className="p-2 space-y-1">
      {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
        <div key={i} className="flex items-center gap-2 px-2 py-1.5 animate-pulse">
          <div className="w-6 h-6 rounded-md bg-gray-200 dark:bg-[#2a2e33]" />
          <div
            className="h-4 bg-gray-200 dark:bg-[#2a2e33] rounded flex-1"
    ...
    <div className="w-[260px] h-full flex flex-col bg-white dark:bg-[#1a1d21] border-r border-gray-200 dark:border-[#2a2e33]">
      ...
      <div className="p-3 border-b border-gray-200 dark:border-[#2a2e33]">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-8 py-2 bg-gray-50 dark:bg-[#25282c] border border-gray-200 dark:border-[#2a2e33] rounded-lg text-sm text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-[#6b7280] focus:outline-none focus:border-brand-500/50 transition-colors"
          />
          {searchQuery && (
            <button
              onClick={() => setSearchQuery('')}
              className="absolute right-2 top-1/2 -translate-y-1/2 p-1 rounded hover:bg-gray-200 dark:hover:bg-[#3a3e43] text-gray-500 dark:text-[#6b7280] hover:text-gray-900 dark:hover:text-white"
            >
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-center p-6">
            ...
            <p className="text-sm text-gray-600 dark:text-[#9ca3af] mb-1">No conversations yet</p>
              ...
              {expandedSections.has('channels') && (
                <div className="px-2 space-y-0.5">
                  {filteredChannels.projectChannels.length > 0 ? (
                    ...
                    <p className="px-2 py-3 text-xs text-gray-500 dark:text-[#6b7280] text-center">
                  ...
                  {!searchQuery && (
                    <button
                      onClick={onCreateChannel}
                      className="w-full flex items-center gap-2 px-2 py-1.5 rounded-lg text-sm text-gray-600 dark:text-[#6b7280] hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-[#25282c] transition-colors"
                    >
                      <Plus className="w-4 h-4" />
      ...
      {contextMenu && (
        <>
          ...
          <div
            className="fixed z-50 w-48 bg-white dark:bg-[#25282c] border border-gray-200 dark:border-[#3a3e43] rounded-xl shadow-xl py-1 overflow-hidden"
            style={{
